...
class MyoLegsGailEnv(BaseEnv):
    def __init__(self, cfg):
        ...
        self.step_counter = 0
        self.latched_ankle_action = None
        self.delta_ankle_action = np.zeros(3)
    ...
    def setup_configs(self, cfg) -> None:
        ...
        self._kd_scale = cfg.env.get("kd_scale", 1.0)
        self.control_mode = cfg.run.get("control_mode", "PD")
    ...
    def physics_step(self, action: np.ndarray = None) -> None:
        ...
        if self.impedance_control:
            # 1. Multi-rate control logic: update ankle actions every 10 steps
            if self.step_counter % 10 == 0:
                # The agent outputs muscles + 3 ankle parameters
                # Identify where the ankle actions start in the action vector
                muscle_size = len(self.muscle_idx)
                new_ankle_action = action[muscle_size : muscle_size + 3]
                
                if self.latched_ankle_action is not None:
                    self.delta_ankle_action = new_ankle_action - self.latched_ankle_action
                else:
                    self.delta_ankle_action = np.zeros(3)
                
                self.latched_ankle_action = new_ankle_action
            else:
                self.delta_ankle_action = np.zeros(3) # Penalty only on change step
            
            self.step_counter += 1

            # 2. Setup Actuator Metadata (Ankle only)
            ...
            # 3. Determine K, B, and Target Position
            if self.active_gains:
                # Use latched action
                imp_ankle = self.latched_ankle_action
            ...
            torque_ankle = K_ankle_raw * (target_ankle - q_ankle) - B_ankle_raw * v_ankle
        ...
        for i in range(self.control_freq_inv):
            if not self.paused:
                ...
                if self.control_mode == "PD":
                    muscle_activity = target_length_to_activation_custom(target_lengths, self.mj_data, self.mj_model, self.muscle_idx)

                    # MANUAL MUSCLE DEACTIVATION
                    if self.cfg.run.get("deactivate_muscles", False):
    ...
    def deactivate_muscles(self, muscle_activity: np.ndarray, targetted_muscles: List[str]) -> np.ndarray:
        ...
        """
        indexes = [self.actuator_names.index(muscle) for muscle in targetted_muscles if muscle in self.actuator_names]
        # Map back to muscle_activity relative indexing if necessary, 
        # but here ctrl[self.muscle_idx] was used. 
        # Let's adjust this to work with the updated indices.
        for idx in indexes:
            if idx in self.muscle_idx:
                muscle_activity[self.muscle_idx.index(idx)] = 0.0
...
def target_length_to_force_custom(lengths: np.ndarray, data, model, muscle_idx) -> list:
    """
    Converts target muscle lengths to forces using a PD control law for specified muscles.
    """
    forces = []
    for i, idx_actuator in enumerate(muscle_idx):
        length = data.actuator_length[idx_actuator]
        velocity = data.actuator_velocity[idx_actuator]
        peak_force = model.actuator_biasprm[idx_actuator, 2]
        kp = 5 * peak_force
        kd = 0.1 * kp
        force = (kp * (lengths[i] - length) - kd * velocity)
        clipped_force = np.clip(force, -peak_force, 0)
        forces.append(clipped_force)

    return forces

def target_length_to_activation_custom(lengths: np.ndarray, data, model, muscle_idx) -> np.ndarray:
    """
    Converts target lengths to activation levels via force computation for specified muscles.
    """
    forces = target_length_to_force_custom(lengths, data, model, muscle_idx)
    activations = []
    for i, idx_actuator in enumerate(muscle_idx):
        length = data.actuator_length[idx_actuator]
        lengthrange = model.actuator_lengthrange[idx_actuator]
        ...
        bias = mujoco.mju_muscleBias(length, lengthrange, acc0, prmb)
        gain = min(-1, mujoco.mju_muscleGain(length, velocity, lengthrange, acc0, prmg))
        activations.append(np.clip((forces[i] - bias) / gain, 0, 1))

    return np.array(activations)

def action_to_target_length_custom(action: np.ndarray, model, muscle_idx) -> list:
    """
    Maps actions to target lengths for specified muscle actuators.
    """
    target_lengths = []
    for i, idx_actuator in enumerate(muscle_idx):
        hi = model.actuator_lengthrange[idx_actuator, 1]
        lo = 0
        target_lengths.append((action[i] + 1) / 2 * (hi - lo) + lo)
    return target_lengths